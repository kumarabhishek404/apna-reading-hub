...
### HTML Prototype as Design Reference (Frontend Tasks)

If Phase 2 created HTML prototype files under `docs/features/{feature-slug}/prototype/`, use them as the visual reference for frontend implementation. Before writing any frontend component:

1. **Read the matching prototype file** for the screen this component belongs to.
2. **Extract the layout structure**, semantic sections, interaction states, and content expectations.
3. **Translate the prototype into app components** by:
   - preserving the overall structure and copy flow
   - replacing static placeholder content with dynamic props and state
   - wiring event handlers and API calls
   - splitting the page into the component boundaries defined in the implementation plan

4. **Keep styling consistent with the existing app** — use the repository’s established patterns rather than introducing a new visual system. Only diverge when the prototype requires a new pattern that the current app already uses elsewhere.
