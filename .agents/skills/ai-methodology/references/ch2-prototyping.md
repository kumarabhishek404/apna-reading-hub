...
## HTML Prototype Drafts

Use plain HTML, CSS, and minimal JavaScript to create a working prototype directly in the feature docs folder. This keeps the workflow lightweight and aligned with the repository’s current approach.

### Prerequisites
- Prototype specification completed (from Approaches 1–4 above)
- A clear feature scope and the relevant screens from the spec

### Sub-step 1 — Create Prototype Files

Create one or more HTML files in `docs/features/{feature-slug}/prototype/`.

Recommended structure:
- `index.html` for the main flow
- additional HTML files for key screens or state variants
- a simple shared stylesheet if needed

### Sub-step 2 — Capture Key States

For each screen in the prototype, include the main interaction states:
- Default
- Empty
- Loading
- Success
...
Use semantic HTML so the prototype is easy to review and hand off.

### Sub-step 3 — Link Prototype to Implementation

Add references to the prototype files in `2-prototype.md` under a "## HTML Prototype Files" section.

Include:
- the path to each HTML file
- what screen or flow it represents
- any interaction notes that are important for implementation

### Quality Checklist for HTML Prototypes
- [ ] Prototype files exist under `docs/features/{feature-slug}/prototype/`
- [ ] Each major screen or flow is represented
- [ ] Primary interaction states are visible or described
- [ ] Prototype references the implementation target paths in the repo
- [ ] The prototype is simple enough to review quickly in a browser
